...
Implement the ETL service (found in `etl/`) such that events are successfully 
propagated from the services that produce events, and into the PostgreSQL database
representing our 'Data Warehouse'. 

- This project contains a docker-compose based setup (see `docker-compose.yml`), 
...
want to read through their code to understand them and their events.
